...
export default function addGridKeyboardNavigation(gridElement: Element) {
    ...
    const focusChild = (dir: "up" | "down" | "left" | "right" | "up") => {
      ...
      if (
        ...
        const index = positions.findIndex(it => it.element.contains(document.activeElement));

        switch (dir) {
          case "right": {
            focus(positions.find(it => it.x === positions[index].x + 1 && it.y === positions[index].y)?.element);
            break;
          }
          case "left": {
            focus(positions.find(it => it.x === positions[index].x - 1 && it.y === positions[index].y)?.element);
          ...
          case "up": {
            focus(positions.find(it => it.y === positions[index].y - 1 && it.x === positions[index].x)?.element);
            break;
          }
          case "down": {
            focus(positions.find(it => it.y === positions[index].y + 1 && it.x === positions[index].x)?.element);
