  ...
  return (
    <Container>
      <Label>
        <InputField
          $color={color}
          $aboveColor={props.labelAboveColor ?? color}
...
const InputField = styled.input<{ $forPassword: boolean; $colored: boolean, $color: string, $aboveColor: string }>`
  font-size: ${fontSize};
  padding: ${padding};
...
const LabelText = styled.div<{ $hasContent: boolean, $color: string, $aboveColor: string }>`
  position: absolute;
  font-size: ${fontSize};
  padding: ${padding};
  pointer-events: none;
  color: ${p => {
    if (p.$hasContent) return p => p.$aboveColor;
    return p => Color(p.$color).mix(Color("#999"), 0.7).hex();
  }};
  top: 50%;
  left: 3px;
