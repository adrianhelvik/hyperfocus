...
export default withConfirm(function DecksList({ confirm }) {
  const [indeces, setIndeces] = useState<{ from: number | null; to: number | null }>({
    from: null,
    to: null,
  });
  ...
  const move = (fromIndex: number, toIndex: number) => {
    ...
    if (item instanceof PortalModel)
      item = { type: "portal", portalId: item.portalId } as any;
    else if (item instanceof DeckModel)
      item = { type: "deck", deckId: item.deckId } as any;
    else {
